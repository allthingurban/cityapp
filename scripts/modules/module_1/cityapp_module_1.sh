...
# If user would create a such map, empty maps will automatically overwritten
   grass $GRASS/$MAPSET --exec  v.edit map=m1_via_points tool=create
   grass $GRASS/$MAPSET --exec  v.edit map=m1_to_points tool=create
   grass $GRASS/$MAPSET --exec  v.edit map=m1_stricken_area tool=create
...
########################################################
## REMOVE (OR COMMENT OUT) EVERITHING FROM THIS POINT ##
########################################################
...
#########################################################
## ADD THE FOLLOWING LINES TO EXPORT A VECTOR TIME MAP ##
#########################################################

    # Converting the result into vector point format
        CONVERSION_RESOLUTION=0.0005
        grass $GRASS/$MAPSET --exec g.region res=$CONVERSION_RESOLUTION
        grass $GRASS/$MAPSET --exec r.to.vect input=m1_time_map output=m1_time_map type=point column=data --overwrite
        grass $GRASS/$MAPSET --exec v.out.ogr -s input=m1_time_map@module_1 type=point output=$GEOSERVER/m1_time_map.gpkg --overwrite
...
# Generating pdf output
    ...
    grass $GRASS/$MAPSET --exec r.colors -a map=m1_time_map color=gyr
    grass $GRASS/$MAPSET --exec r.colors map=m1_time_map_interpolated color=gyr
    
    
    # BUG IN COLORS (CHECKING COLOR_RULES)
    # BUG IN color when via points is added
